...
export const generateCleanedLineSplitArr = (data: string): LineSplitObj[] => {
  const newLineSplitArr = data.split("\n").map((l: string, i: number) => {
    ...
    const result = {
      line: i + 1, // line numbers start at 1 not at index 0.
 ...
 */
export const sortCleanedIndexArr = (
  inputArr: SearchIndexEntryArr[]
): SearchIndexEntryArr[] => {
...
export const reformatIndex = (
  cleanedAndSortedIndex: SearchIndexEntryArr[]
): SearchIndexEntryObj[] => {
  return cleanedAndSortedIndex
    .map((el: SearchIndexEntryArr) => {
        return {
          searchTerm: el[0],
          searchResults: el[1],
        };
